...
const servers = [
  { id: 1, name: 'HolyWorld', ip: 'mc.holyworld.ru', online: 8421, max: 12000, ping: 42, version: '1.16–1.21', tags: ['Анархия', 'Java'], bid: 480, mark: 'HW', tone: '#c97042', desc: 'Кланы, ивенты и свобода действий', wipe: 'Вайп 2 дня назад', motd: ['HOLYWORLD', ' ЛЕТНИЙ СЕЗОН · /FREE'] },
  { id: 2, name: 'FoxMine', ip: 'play.foxmine.net', online: 6190, max: 8000, ping: 38, version: '1.20.4', tags: ['Выживание', 'Без доната', 'Bedrock+Java'], bid: 390, mark: 'FM', tone: '#c4902f', desc: 'Честное выживание и кроссплей', wipe: 'Вайп сегодня', motd: ['FOX MINE', ' НОВЫЙ МИР УЖЕ ОТКРЫТ'] },
  { id: 3, name: 'ReallyWorld', ip: 'mc.reallyworld.ru', online: 4872, max: 10000, ping: 51, version: '1.16.5', tags: ['Анархия', 'PvP'], bid: 270, mark: 'RW', tone: '#7159c7', desc: 'Классическая анархия без лишних правил', wipe: 'Вайп 5 дней назад', motd: ['REALLYWORLD', ' ГЛОБАЛЬНОЕ ОБНОВЛЕНИЕ'] },
  { id: 4, name: 'Vanilla Plus', ip: 'vanilla.plus', online: 1248, max: 2000, ping: 29, version: '1.21', tags: ['Выживание', 'Без доната', 'Голосовой чат'], bid: 0, mark: 'V+', tone: '#3b9b72', desc: 'Новая ваниль с близким комьюнити', wipe: 'Без вайпов', motd: ['VANILLA+', ' ЧЕСТНАЯ ИГРА БЕЗ P2W'] },
  { id: 5, name: 'CubeCraft RU', ip: 'ru.cubecraft.gg', online: 956, max: 3000, ping: 64, version: '1.19–1.21', tags: ['Мини-игры', 'Bedrock+Java'], bid: 0, mark: 'CC', tone: '#3976bd', desc: 'Быстрые мини-игры для компании', wipe: 'Вайп неделю назад', motd: ['CUBECRAFT', ' BEDWARS · SKYWARS · ARCADE'] },
];

const filters = ['Все', 'Анархия', 'Выживание', 'Без доната', 'Голосовой чат', 'Bedrock+Java'];

function ServerRow({ server, rank }: { server: typeof servers[number]; rank: number }) {
  ...
  return <article className="server-row">
    ...
    <div className="server-main">
      ...
      <div className="server-copy">
        <div className="name-line"><h3>{server.name}</h3></div>
        <p>{server.desc}</p>
        <div className="tags">{server.tags.map(tag => <span key={tag}>{tag}</span>)}</div>
      </div>
    </div>
...
export default function Home() {
  ...
  return <main>
    <header className="site-header">
      ...
      <nav className={menu ? 'open' : ''}><a href="#rating">Все серверы</a><a href="#rating">Новые</a><a href="#rating">Bedrock</a></nav>
      <div className="header-actions"><button className="sign-in">Войти</button><button className="add-server">Добавить сервер <ArrowRight /></button><button className="menu-button" aria-label="Меню" onClick={() => setMenu(!menu)}>{menu ? <X /> : <Menu />}</button></div>
    </header>
    ...
    <section className="rating" id="rating">
      ...
      <div className="server-list">{visible.length ? visible.map((server, index) => <ServerRow key={server.id} server={server} rank={index + 1} />) : <div className="empty"><Search /><h3>Ничего не нашли</h3><p>Попробуй другой режим или сбрось фильтры.</p><button onClick={() => { setFilter('Все'); setQuery(''); }}>Сбросить</button></div>}</div>
      <button className="more">Показать ещё 24 сервера <ArrowRight /></button>
    </section>

    <footer><a className="brand" href="#top"><img src="/minecraft-rating-mark.png" alt="" /><span>MINECRAFT <b>RATING</b></span></a><p>Честный мониторинг серверов Minecraft.</p><span>© 2026</span></footer>
  </main>;
}
